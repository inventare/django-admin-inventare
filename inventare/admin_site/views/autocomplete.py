...
class HTMXAutocompleteView(BaseListView):
    paginate_by = 10
    admin_site = None
    template_name = None
    ...
    def process_request(self, request):
        ...
        try:
            self.app_label = request.GET["app_label"]
            self.model_name = request.GET["model_name"]
            self.field_name = request.GET["field_name"]
        except KeyError as e:
            raise PermissionDenied from e

        # Retrieve objects from parameters.
        try:
            source_model = apps.get_model(self.app_label, self.model_name)
